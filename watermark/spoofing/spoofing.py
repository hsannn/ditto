    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.config.enable_part_1:
            for b_idx in range(input_ids.shape[0]):
                ...
                score_values_list = torch.tensor(list(score_list.values()), device=scores.device, dtype=scores.dtype)

                # indices = token_ids.to(torch.long)
                scores[b_idx, token_ids.to(torch.long)] += (score_values_list * self.config.delta * weight)
