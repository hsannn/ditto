...
class MeanDetector(SynthIDDetector):
    ...
    def detect(self, g_values, mask):
        ...
        score = torch.sum(torch.tensor(g_values) * torch.tensor(mask).unsqueeze(2), dim=(1, 2)) / (
            watermarking_depth * num_unmasked)

        z_score = (score - 0.5) * torch.sqrt(torch.tensor(4 * watermarking_depth * num_unmasked, dtype=torch.float64))

        # p_value = torch.special.erfc(z_score/torch.sqrt(torch.tensor(2.0, dtype=torch.float64)))/2
    ...
    def _compute_extreme_p_value(self, z_score):
        mpmath.mp.dps = 10
        # Convert tensor to float before passing to mpmath
        z = mpmath.mpf(float(z_score))
    ...
    def _compute_p_value(self, z_score):
        if z_score > 38: 
            ...
            # 对于较小的z_score仍使用常规方法
            p_value = 0.5 * torch.erfc(torch.tensor(z_score, dtype=torch.float64) / torch.sqrt(torch.tensor(2.0)))[0]
        
        return p_value
    
    def detect_no_p_value(self, g_values, mask):
        """
        Args:
            g_values: shape [batch_size, seq_len, watermarking_depth]
            mask: shape [batch_size, seq_len]
